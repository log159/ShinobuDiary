...
void CubismShader_D3D11::GenerateShaders(ID3D11Device* device)
{
    ...
    do
    {
        ...
        if (!LoadShaderProgram(device, true, ShaderNames_NormalMaskedInverted, const_cast<csmChar*>("PixelMaskedInverted")))
        {
            break;
        }
        if (!LoadShaderProgram(device, true, ShaderNames_NormalPremultipliedAlpha, const_cast<csmChar*>("PixelNormalPremult")))
        {
            break;
        }
